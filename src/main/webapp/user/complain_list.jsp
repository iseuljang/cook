					...
					while(rs.next()){
						...
						<tr>
							...
							<td>
								...
								href="<%= request.getContextPath() %>/board/view.jsp?type=<%= board_type %>&no=<%= boardNo %>&nowPage=<%= nowPage %>&searchType=<%= searchType %>&searchValue=<%= searchValue %>">
								<%= rs.getString("title") %>
								<%
