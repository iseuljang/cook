...
try {
    ...
    fname = filename; // 업로드된 파일 이름
    pname = phyname; // 원본 파일 이름 (이름이 필요하면 따로 저장하도록 설정)
	System.out.println("pname:" + pname + ", fname:" + fname);
    ...
    // 파일이 첨부된 경우 파일 정보를 업데이트
    if (fname != null) {
    	String sqlFileSelect = "select * from attach where bno=?";
    	psmt = conn.prepareStatement(sqlFileSelect); //사용할 쿼리 등록
    	...
    	if(rs.next()){
	        ...
	        psmt.setString(2, pname); // 업로드된 파일의 원본 이름을 설정
	        psmt.setInt(3, Integer.parseInt(no));
	        psmt.executeUpdate();
	        
	        System.out.println("SQL 쿼리: " + sqlFileUpdate + ", fname: " + fname + ", pname: " + pname + ", bno: " + no);
	        
	        int fileUpdateResult = psmt.executeUpdate();
	        System.out.println("파일 업데이트 결과: " + fileUpdateResult);
    	}else{
    		String sqlFileInsert = "insert into attach (fname,pname,bno) "
    				+ "values (?,?,?) ";
	        ...
	        psmt.setInt(3, Integer.parseInt(no));
	        psmt.executeUpdate();
	        System.out.println("SQL 쿼리: " + sqlFileInsert);
    	}
    }
